...
4. Оновіть сторінку магазину або натисніть меню (три крапки) → **Перевірити оновлення (Check for updates)**.
5. У списку репозиторіїв знайдіть **UZ Train Delay Monitor** і натисніть **Встановити (Install)**.
...
``
Якщо тільки один користувач: У правому верхньому кутку сторінки конфігурації аддона натисніть меню (три крапки) → Edit in YAML (Редагувати в YAML) і вкажіть ID списком із дефісом:
```
tg_chat_ids:
...
1. Створити користувача в Home Assistant

    У бічному меню Home Assistant перейдіть:

    Налаштування (Settings) → Люди (People) → вкладка Користувачі (Users) (якщо вкладки немає — увімкніть «Розширений режим» у своєму профілі користувача внизу зліва).

    Натисніть Додати користувача (праворуч знизу):

        Ім'я: mqtt-user

        Ім'я користувача: mqtt-user

        Пароль: придумайте пароль (наприклад, MyMqttPass123)
'
        Перемикач «Дозволити вхід тільки з локальної мережі» можна ввімкнути.
'
    Натисніть Створити.
'
    Тепер відкрийте конфігурацію аддона UZ Train Delay Monitor і вкажіть там цей логін та пароль:
    YAML

    mqtt_user: "mqtt-user"
    mqtt_password: "MyMqttPass123"
   
2. Додавання MQTT-сенсора
...
3. Скидання стану вночі (Автоматизація)
